...
from deepscm_vae import mnist as image_mnist
...
if __name__ == '__main__':
    ...
    vae = image_mnist.MorphoMNISTVAE(latent_dim=args.latent_dim).to(device)
    ...
    t_dist, i_given_t_dist, s_dist = attribute_mnist.load_model(args.attr_model_file,
                                                                device=device)
    _, counts = a_train[:, 10].argmax(dim=1).unique(return_counts=True)
    counts = counts / counts.sum()  # p(label)
    ...
    with torch.no_grad():
        ...
        pred_image_out = 0
        for _ in range(32):
            pred_image_out = pred_image_out + vae.decoder(vae.encoder.sample(x, c), c_cf)
        pred_image_out = pred_image_out / 32.0
        ...
        axs[0].set_ylabel('DeepSCM Measured')
