...
class RandomThickening(Perturbation):
    def __init__(self,
                 thickening_bounds=(0.5, 1.5)):
        self.thickening_bounds = thickening_bounds
        self.last_thickness = None
    
    def __call__(self, morph: ImageMorphology) -> np.ndarray:
        level = np.random.uniform(*self.thickening_bounds)
        self.last_thickness = level
        return Thickening(level)(morph)
