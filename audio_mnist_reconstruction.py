...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        real_audio = data.inv_transforms["audio"](batch["audio"][0:1].cpu().numpy()).cpu().numpy()
        bigan_audio = data.inv_transforms["audio"](img_to_spect(bigan_rec).cpu().numpy()).cpu().numpy()
        bigan_ft_audio = data.inv_transforms["audio"](img_to_spect(bigan_ft_rec).cpu().numpy()).cpu().numpy()
        vae_audio = data.inv_transforms["audio"](img_to_spect(vae_rec).cpu().numpy()).cpu().numpy()
        ...
        axs[1, 1].plot(bigan_audio.reshape((-1,)))

        axs[0, 2].imshow(bigan_ft_rec[0].cpu().reshape((128, 128)).numpy()[::-1], vmin=-1, vmax=1)
        axs[0, 2].set_xticks([])
        axs[0, 2].set_yticks([])
