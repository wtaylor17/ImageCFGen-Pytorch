...
import numpy as np
import os
from deepscm_vae.audio_mnist import ATTRIBUTE_DIMS, AudioMNISTData, VAE, VALIDATION_RUNS
...
if __name__ == "__main__":
    ...
    vae: VAE = torch.load(model_path, map_location=device)["vae"].to(device)
    ...
    with torch.no_grad():
        ...
        for attr_batch in batchify_dict(attrs, device=device):
            ...
            for _ in range(mc_rounds):
                ...
                print(z.shape, attr_batch["gender"].shape)
                gen = gen + vae.decoder(z, attr_batch)
            gen = gen / mc_rounds
